...
// isBase  : true = 「拠点として認識される」。targets:'base' のユニットもこれを攻撃する。
// img     : 画像相対パス（文字列）。省略時は絵文字(e)を使用。
...
var PLAYER_UNITS = {
  ...
  // isBase:true — targets:'base' の敵もこのユニットを「拠点」として攻撃する
  // img — 画像テスト用（他のユニットは引き続き絵文字）
  golem:    { n:'ゴーレム',   e:'🛡️', img:'画像/golem.png', cost:50, hp:420, dmg:15, spd:24, rng:30, ar:1.3, type:'ground', targets:'ground', isBase:true, cd:3.0, size:1.5, rew:2, zone:'own', unlockCost:300 },
...
function mkUnit(defId, team, x, y, d) {
  return {
    ...
    team    : team,
    e       : d.e,
    img     : d.img    || null,    // 画像パス（null なら絵文字）
    nm      : d.n,
    hp: d.hp, mhp: d.hp,
    ...
    type    : d.type,
    targets : d.targets,
    isBase  : d.isBase || false,   // 拠点として認識される
    size    : d.size   || 1.0,
    x: x, y: y,
